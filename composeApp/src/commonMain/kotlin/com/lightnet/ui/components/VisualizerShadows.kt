...
internal fun DrawScope.drawInCanvasShadow(
    ...
    when (shadow.implementation) {
        ShadowImplementation.Layered -> drawLayeredShadow(path, shadow)
        ShadowImplementation.Feathered -> drawFeatheredShadow(points, shadow, cornerRadius)
...
private fun DrawScope.drawLayeredShadow(path: Path, shadow: ShadowConfig) {
    val n = shadow.layered.layers.coerceAtLeast(1)
    for (i in n downTo 1) {
        val f = i.toFloat() / n
        translate(shadow.offsetX * f, shadow.offsetY * f) {
 ...
 */
private fun DrawScope.drawFeatheredShadow(points: List<Offset>, shadow: ShadowConfig, cornerRadius: Float) {
    val steps = shadow.feathered.steps.coerceAtLeast(1)
    val ringAlpha = shadow.color.alpha / steps
    translate(shadow.offsetX, shadow.offsetY) {
        for (s in steps downTo 1) {
            val grow = shadow.feathered.blur * (s.toFloat() / steps)
 ...
 */
internal fun DrawScope.drawNativeBlurShape(path: Path, shadow: ShadowConfig) {
    translate(shadow.offsetX, shadow.offsetY) {
 ...
 */
internal fun DrawScope.drawInnerShadow(path: Path, inner: InnerShadowConfig) {
    ...
    clipPath(path) {
        for (s in 1..steps) {
            val width = inner.width * 2f * (s.toFloat() / steps)
